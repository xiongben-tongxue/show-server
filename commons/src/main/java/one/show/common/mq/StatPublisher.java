...
public class StatPublisher {
	...
	private static Logger logger = LoggerFactory.getLogger(StatPublisher.class);
	
	private static StatPublisher publisher = new StatPublisher();
	...
	public final static StatPublisher getInstance() {
		return publisher;
	}
	...
	public void sendMessage(final String message, final Queue queue) throws JMSException {
		...
		new AsyncHandler(){
			@Override
			public void task() {
				logger.info("send monitor or stat start cost:"+(System.currentTimeMillis()-start)+"ms");
				try {
...
//					producer.send(destination, textMessage);  
		...
		}.handle2("send message to mq : "+ message);
	...
	public static void main(String[] args) {
		for(int i=0; i<10; i++){
			try {
				StatPublisher.getInstance().sendMessage("test+"+i, Queue.TEST);
